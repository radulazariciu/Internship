...
Function Get-WebConfigs($solution)
{
	# Get the search path to the config files that starts with the connection name.
	$solutionDir = Split-Path -parent $solution.FullName
	
	# Search in the solution for the mdf file, and return the first hit.
	return Get-ChildItem $solutionDir -Recurse -Include "web.config"
}

Function Get-CommerceManagerLinkElement($config)
{
	return $config.SelectSingleNode("//appSettings/add[@key='CommerceManagerLink']")
}

Function Set-CommerceManagerLink($webConfigInfos, $commerceManagerLink)
{
	foreach($webConfigInfo in $webConfigInfos)
	{
		$config = Get-Config $webConfigInfo.FullName
		$commerceManagerLinkElement = CommerceManagerLinkElement $config
		
		if($commerceManagerLinkElement -ne $null)
		{
			$attributeValue = $commerceManagerLinkElement.GetAttribute("value");

			if([string]::IsNullOrWhiteSpace($attributeValue) -eq $true)
			{
				$commerceManagerLinkElement.SetAttribute("value", $commerceManagerLink) | Out-Null
				$config.Save($webConfigInfo.FullName)
			}
		}
	}
}

Function Get-CommerceManagerLink($project)
{
	$config = Get-Config $project.FullName
	...
	$ns.AddNamespace("e", $xmlNamespace)
	$element = $config.SelectSingleNode("//e:Project/e:ProjectExtensions/e:VisualStudio/e:FlavorProperties/e:WebProjectProperties/e:IISUrl", $ns)
	if($element -eq $null)
	{
		return $null;
	}
...
Function Update-CommerceManagerLink($solution, $project) 
{
	if($solution -eq $null -or $project -eq $null)
	{
		return $null;
	}

	$webConfigInfos = Get-WebConfigs $solution
	if($webConfigInfos -eq $null)
	{
		return $null;
	}
	
	$commerceManagerLink = Get-CommerceManagerLink $project
	Set-CommerceManagerLink $webConfigInfos $commerceManagerLink
}

Export-ModuleMember -Function Update-CommerceManagerLink
# SIG # Begin signature block
# MIIZDwYJKoZIhvcNAQcCoIIZADCCGPwCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUOn9wcknWMFSWUOs0L8KaMJ+c
# cZCgghP/MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBRNphmx2o7SQoKICljZvEhExBAm
# iTANBgkqhkiG9w0BAQEFAASCAQBWeYdOrvenk1QSWEh0orX/AZHjNd1O0DtHCxQD
# 7D03ThudnRqet3DlvyD1shHXJKclPKa/CPjm31bSvo0jwnmmXfMu9gynNxhfzfjO
# Kfu3nY12D4kOnMJqthakXOg4uElQGG2Q8kCZ4pJgJ96FJVDCaHseSHejM+IBVn0u
# FINPGSP5YoIpVnMOYeBAyrAldzB4J+lNjwBfSDVJMY1kTdrw899WtToea4Sn5H4E
# OAzzViU6IoLXF7dE6fp4aRPHVNZfOMQBXhxQa829/k4bKAQeEYAZuuf74/FAmlfG
# LqR1uBz1XNsJSdt95wVJ6mNFvFH77uWILkxfynJBRU0QDglfoYICCzCCAgcGCSqG
...
# MDcwNzE0NDAzN1owIwYJKoZIhvcNAQkEMRYEFEb/2sr3eQ487MJkDn61VirtNMtj
# MA0GCSqGSIb3DQEBAQUABIIBAJYFZbUpaqiS5t/uHfDoaUQNkIrGY3Hiwr9SLdBW
# bDroDZiwuENyS7odjnoEsEALkedzoyl56pBUy988PgZcfay7/5YHu6KyWjJT+b83
# Ig5L1Tw5wE8FKGPsDJoWx+3I7T7PZKS9choa1gwrLWsRipSCBEXTVqu0O22r9wtF
# OETf19Ovoqt84M2oGt6DfPEhz2C/7YgRVZAEQneGlWlNeQPbex68ZCp6VHyJgNXS
# yP21cNr870zsOxXihYnXSQqTfC/s19qLw3Nv8fC89cXVoNRyUdxkhgr5u+UbYemQ
# xCcsHUrInknwwS67JN18S1/nwxBXvD+fO76B+guQiTXWW6w=
