...
#>
	[CmdletBinding()]
    param ([string]$sqlFilePattern, [string]$connectionString,[bool]$validation = $false)
	...
	{
		# deploy all products
		DeploySqlFiles $epideploy $packages $packagePath $sitePath $validation
		return
	}
	...
	if ($sqlFilePattern)
	{
		DeploySqlFile $epideploy $connectionString $sqlFilePattern $sitePath $validation
		return;	
	}
}
...
{
 	[CmdletBinding()]
	 param ($epideploy, $packages, $packagesPath, $sitePath, [bool]$validation = $false)
	 ...
	 $packages | foreach-object -process {
			...
			if (test-Path $sqldatabaseFile){
				...
				DeploySqlFile $epideploy $packageSetting["connectionStringName"] $sqldatabaseFile  $sitePath  $validation
			}
		}
}
...
{
	[CmdletBinding()]
	param ($epideploy, [string]$connectionString, [string]$sqlFilePattern, [string]$sitePath, [bool]$validation = $false)
	...
	{
		&$epideploy  -a "sql" -s $sitePath  -p $sqlFilePattern -b  $connectionString  -v $validation -d (GetVerboseFlag($PSBoundParameters)) 
	}
	else
	{
		&$epideploy  -a "sql" -s $sitePath  -p $sqlFilePattern -c  $connectionString  -v $validation -d (GetVerboseFlag($PSBoundParameters))
	}
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU1bpB8wJIXdQk2aGO9rknBAj7
# 45+gghP/MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBT5YCqP9A7hk1AFX4V6jjJ8ZiOR
# ODANBgkqhkiG9w0BAQEFAASCAQA6tM+4USYPSHvfDkZm/upVMWkD0z8wpjcVGmzy
# ZvQyWWdQQNqaXnZicHsd+QysbvDKfKWwQd+QqaUazIuJKm7qnXfUH7mAVgeerQR6
# EfemuY5F6TfuxOiIMnFTS4lEcCXsSqkHdSfHluIza1brzq2vTIINdBo1kF4nVjnu
# dRxzPuYHLLDh4lQZNYU1jX4ci8KetJxeMFiNFZJiyMm8Lt4JBi4XLew1FkFmbXCS
# kPLMDDc8KZqNFEhYd5AryNrDrOsd0y4k+Nz2g+VaUNpCV4eHQmsZdqy02V1lWueX
# rnWiWOxkY819tqnADYGliY5n3k/V7DyGdkj5wTjTg++cYdfBoYICCzCCAgcGCSqG
...
# MDYyNjEwNDQyMlowIwYJKoZIhvcNAQkEMRYEFPYrahBGk8wxr+wZVgI9+gjhQm2h
# MA0GCSqGSIb3DQEBAQUABIIBAHt4d+rDwzGE35to7qeKyLjjD/kmHClK189biiAf
# 9oo0sElkUPb5sJejjem3i4rLwFEwttG0W9waCSkd+Xg73nQFn1Bj8ldecPnoF8Rw
# M8YMpp/iapWZAkLLBkDeNclscUfgAXKF6PNNNlU8EMxJeB8k1hb7Utl/e3YeCJdJ
# tBALQdKr1juqp3rEKOxkClo6BZDNNRLTzjI+HJSZGvlcjtLomQw925f8Pu+WRIwk
# JMQCV3DEwGpSReoE2aJP51q0ieqqH81PhB7Io8yOFfVN0bG/keZx+l1XbQmaWKkK
# vaH7kUA7N/izRG5pFhB7yABy+DAnuRjG6Aqh76gwfXP4HUU=
