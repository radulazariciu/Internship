...
#import Module
Import-Module (Join-Path (join-path $installPath  "tools")  Get-EPiServerCommerceConnectionString.psm1)
...
Import-Module (Join-Path (join-path $installPath  "tools")  Copy-DB.psm1)
Import-Module (Join-Path (join-path $installPath  "tools")  Set-NameSpace.psm1)
Import-Module (Join-Path (join-path $installPath  "tools")  Copy-EcfConfigsIfNotExist.psm1)
Import-Module (Join-Path (join-path $installPath  "tools")  Set-CommerceManagerSearchIndexBasePath.psm1)
...
if ($epiConnection -eq $null)
{
	...
	$episerverWebConfig = Join-Path (join-path $installPath  "tools")  "EPiServer.Commerce.config"
	# Deploy Web Config
	...
	$thePackage = Get-package -ProjectName $project.ProjectName | where-object { $_.id -eq "EPiServer.Commerce.Core"} | Sort-Object -Property Version -Descending | select-object -first 1
	$commerceCoreToolsPath = Get-PackagesToolPath $installPath "EPiServer.Commerce.Core" $thePackage.Version
	...
	if (-Not (Test-Path $infrastructurePath))
	{
		...
		$project.ProjectItems.AddFromDirectory($infrastructurePath)
	}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUILmRZWsLqp6a+qzxezcPUkkD
# rPugghP/MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQcu+vaLOfGqoVM4nXYjP0Iikq2
# djANBgkqhkiG9w0BAQEFAASCAQBIUuwCGwq2TUDgV3Vr/CeGQ9tVQuIhm9PhHqBQ
# rXEm6KbnYkgUf7LpLGfxSBaCNnamCKEhy25qhYDv1YzZZNdmACxjDHiaWiVX8r1Y
# jTTkQlA/QlTSpYqmvpU0t0hsTfOAE8G7bXVQsNVvwaOIeeaSeHTROtozDn6u+yyK
# sTtKYRm8B75BSCNoIIml3Wmd1/yA3Q2r4G2OmM4TFsR5oxJx9x1mb4Sz9XR5Xir/
# NVhsDAjNh+Ni/wgd/HtP3QSfrOW0poHCikjQuzGbPbj9kzxg4B4Rj1hPjh2q+3b0
# QLoWTIuzbllKlD8gCuUPAaZAun95s16AbYuvM4XxcY/xc2idoYICCzCCAgcGCSqG
...
# MDcwNzE0NDAyOFowIwYJKoZIhvcNAQkEMRYEFE6KfFi+ogy2US3HqeZReFNwNcYx
# MA0GCSqGSIb3DQEBAQUABIIBAEasPPbj2dUeWurYe15P7Vuq3l4GdXJkaghqJqLr
# qNDHLgTee4okiNCBPHBsNCjbJOzotcx17GbjTFewMeetAry66WLHdfIZ5y6pZKbz
# x+flQyPGWJfrhWfIvLCz8ameNeP3WIjjbK0bD5kIqvAyOZfZJKzh71tfUvoXIWXq
# vjJXiWTIaMhuIV/BaKfUIEK5PPEx+xxIfnwEArfa+Y+3K+MOzW3IXqA2b4g1XSnX
# m9fzIorkmcZit+6Jk43RwxhUndlaT5Rik8Hs0UCNy87piZ39zz7gBz/TeWaLisVL
# kdc3jEUxqciHCno0dToNJkz4482tzZkopPQLTBYQ6Zn+A1Q=
