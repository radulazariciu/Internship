...
namespace EPiServer.Commerce.Sample.Templates.Sample.Units.CartCheckout.SharedModules
{
    public partial class ErrorModalModule : UserControlBase
    {
        ...
        {
            Page.ClientScript.RegisterStartupScript(GetType(), "Javascript", "javascript: $('#errormodal').modal('show')"
                + ".css({width: '200px','margin-top': function () {return -($(this).height() / 2);}, 'margin-left': function (){  return -($(this).width() / 2); }});", true);
        }
    }
}
